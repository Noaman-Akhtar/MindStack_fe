...
export function CreateContentModal({
  ...
  async function addcontent() {
    ...
    if (
      resolvedType === ContentType.Twitter &&
      (link === "" || !isValidTwitterUrl(link))
    ) {
  ...
  return (
    <div className="flex flex-col justify-center relative">
      ...
      >
        <div
          className="absolute top-[0.01%] right-[0.01%]  cursor-pointer"
          onClick={onClose}
        >
          <CrossIcon />
        </div>
        <div className="w-[50vm] flex flex-col  gap-y-2">
          ...
          <div className="mt-2  text-md placeholder:italic  ">
            <RichTextEditor
              onDeltaChange={setRichNoteDelta}
              onHtmlChange={setRichNote}
            />
          </div>
          <div className="mt-4">
            <MultiUploader
